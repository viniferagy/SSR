...
def _repeat_past(past, bsz: int):
    ...
    if hasattr(past, "batch_repeat_interleave"):
        new_past = type(past).from_legacy_cache(past.to_legacy_cache()) if hasattr(past, "to_legacy_cache") else past
        try:
            new_past.batch_repeat_interleave(bsz)
            return new_past
        except Exception:
            pass
    if hasattr(past, "layers") and DynamicCache is not None:
        cache_data = []
        for layer in past.layers:
            ...
            if k is None or v is None or k.numel() == 0:
                ...
                cache_data.append((k.expand(bsz, *k.shape[1:]).contiguous(), v.expand(bsz, *v.shape[1:]).contiguous()))
            else:
                cache_data.append((k, v))
...
@torch.no_grad()
def compute_reasoning_token_entropies(model, flat_reason_ids: torch.LongTensor, past_prefix, batch_size: int = 512, prefix_ids: Optional[torch.LongTensor] = None) -> List[float]:
    device = next(model.parameters()).device
    L = int(flat_reason_ids.numel())
    if L == 0: return []
    if past_prefix is None:
        if prefix_ids is None:
            prompt_ids = flat_reason_ids.to(device)
            prompt_len = 0
        else:
        ...
        for i in range(0, L, batch_size):
            reason_start = prompt_len + i
            reason_end = prompt_len + min(i + batch_size, L)
            input_ids = prompt_ids[:reason_end].unsqueeze(0).to(device)
            attention_mask = torch.ones_like(input_ids)
            outputs = model(input_ids=input_ids, attention_mask=attention_mask, use_cache=False)
            logits = outputs.logits[:, reason_start:reason_end, :]
...
def process_single_method(model, tokenizer, device, max_ctx, think_open_ids, think_close_ids, reason_sep_ids, 
                          method: str, obj: Dict, line_idx: int, debug_errors: bool = False) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if prompt_content_c:
            ...
            token_entropies = compute_reasoning_token_entropies(model, flat_n, past_prefix_c, 512, entropy_prefix_ids)
            if token_entropies:
        ...
        if debug_errors:
            print(f"[debug] sample={line_idx} method={method} failed: {type(e).__name__}: {e}", flush=True)
            traceback.print_exc()
        return None
...
def main():
    ...
    parser.add_argument("--debug-errors", action="store_true", help="Print per-method exceptions instead of silently dropping rows")
    parser.add_argument("--disable-shared-prefix-past", action="store_true", help="Avoid cached-prefix batching for newer Transformers cache APIs")
    args = parser.parse_args()

    global USE_SHARED_PREFIX_PAST
    if args.disable_shared_prefix_past:
        USE_SHARED_PREFIX_PAST = False
        ...
        for line_idx, obj in tqdm(samples, desc=f"Processing (rank{rank})", disable=(rank != 0)):
            ...
            for m in methods:
                metrics_record = process_single_method(
                    ...
                    m, obj, line_idx, args.debug_errors
                )
